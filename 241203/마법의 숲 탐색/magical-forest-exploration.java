...
public class Main {
    ...
    static boolean init(int startC){
        
        if(forest[1][startC] != 0 || forest[2][startC] != 0){
            return false;
        }
...
//방향 설정해서 이동하기
    static boolean moveDown(Golem golem){
        ...
        for(int i = 1 ; i < 4; i++){
            if(golem.block[i][0] >= R || golem.block[i][1] >= C ) continue;
    ...
    static boolean moveLeft(Golem golem){
        ...
        // System.out.println("exitDir left" + golem.exitDir);
        return true;
    }
    ...
    static boolean moveRight(Golem golem){
        ...
        // System.out.println("exitDir right " + golem.exitDir);
 ...
 //이동하기
    static int move(int golemNum){
        ...
        return findMaxR(golem, new boolean[R+1][C]);
    }
    
    static int[] exit (Golem golem, int exitDir){
    ...
    public static int findMaxR(Golem golem, boolean[][] visited){
        ...
        int[] exit = exit(golem, golem.exitDir);

        // System.out.println("exitDir " + golem.exitDir);

        for(int i = 0 ; i< 4 ;i++){
            int movedR = golem.exit[0] + dr[i];
            int movedC = golem.exit[1] + dc[i]; 
            
            if(movedR > R || movedR < 0 || movedC >= C || movedC < 0 
    ...
    static void update(Golem golem){
        ...
        for(int i = 0 ; i < 4; i++){
            if(golem.block[i][0] > R || golem.block[i][1] >= C || golem.block[i][0] < 0 || golem.block[i][1] < 0 ) continue;
